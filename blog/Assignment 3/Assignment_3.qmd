...
cat("Dataset dimensions (rows x cols):", paste(dim(conjoint_data), collapse = " x "), "\n\n")
...
The “hard part” of computing the MNL likelihood is organizing the data, since we have to keep track of three dimensions – consumer $i$, product $j$, and covariate/feature $k$ – instead of the typical two dimensions for a cross-sectional regression model (consumer $i$ and covariate $k$). In our case, each task for each respondent has the same number of alternatives (3), which simplifies things. We also need to convert the categorical variables (brand and ads) into numeric binary indicator variables for modeling.
...
<!-- ```{python}
# | echo: false
# legacy alias so later code that still expects `df` keeps working
df = conjoint_df
``` -->
