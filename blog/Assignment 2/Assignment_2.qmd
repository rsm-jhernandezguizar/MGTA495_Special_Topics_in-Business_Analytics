...
**Story in one breath — key Poisson IRRs**

- **Instant Book** → × 1.42 reviews (**+42 %**)  
- **Cleanliness** → +11 % per rating point  
- **Listing age** → +1.9 % per year  
- **Property size & price** → more bedrooms ↑ extra bathrooms ↓ log-price ↑ (modestly)  
- **Room type** → Private > Entire > Shared (~ 10 % edges)
...
* **Flip Instant Book** → bookings jump ≈ 40 %.  
* **Clean like a germaphobe** → +11 % per rating tick.  
* **Price for value:** once value perception is held constant, high-price doesn’t hurt.  
* **Extra bedrooms** help a tad; extra baths alone don’t.  
* Room-type differences are small; shared rooms lag slightly but sample is tiny.  
* Dropping zero-review listings simplified the model; a full funnel study would treat the “will I ever get a review?” hurdle separately.
* Models disagree on tiny details but **agree on the big knobs**. 